...
def solve1(s):
    return sum(is_valid(k) for k in s)
...
assert solve1(parse('aa bb cc dd ee')) == 1
assert solve1(parse('aa bb cc dd aa')) == 0
assert solve1(parse('aa bb cc dd aaa')) == 1
print(solve1(parse(input)))

# part 2

def is_valid(s):
    m = [''.join(sorted(k)) for k in s.split()]

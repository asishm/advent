...
def parse(s):
    l = []
    all_regs = set()
    pat = r'([a-z]+) (inc|dec) (-?\d+) if ([a-z]+) (.+?) (-?\d+)'
    for line in s.split('\n'):
        mod_reg, inc, val_inc, cond_reg, cond_sym, cond_val = re.search(pat, line).groups()
        all_regs.add(mod_reg)
        all_regs.add(cond_reg)
        d = {
            'mod_reg': mod_reg,
            'mod_inc': inc,
            'mod_chg': val_inc,
        ...
        l.append(d)
    return l, {k: 0 for k in all_regs}


def solve1(l, all_regs):
    all_max = 0
    for ins in l:
        command = 'b = all_regs["{}"] {} {}'.format(ins['cond_reg'], ins['cond_sym'], ins['cond_val'])
        exec(command)
        if locals()['b']:
            if ins['mod_inc'] == 'inc':
                all_regs[ins['mod_reg']] += int(ins['mod_chg'])
            else:
                all_regs[ins['mod_reg']] -= int(ins['mod_chg'])
            print(command, locals()['b'], ins['mod_reg'], ins['mod_inc'], ins['mod_chg'])
        else:
            print(command, locals()['b'])
        all_max = max(all_max, max(all_regs.values()))
    return max(all_regs.values()), all_max

inp = """y inc 497 if n <= 3
ig inc -54 if es < 9
j dec 278 if low < 10
nm inc -531 if tr == 0
tq inc 537 if tq < 9

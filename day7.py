...
def parse(s):
    info = []
    for line in s.split('\n'):
        d = {'name': '', 'weight': '', 'balance': ''}
        r = re.search(r'([a-z]+) \((\d+)\)(?: -> (.*))?', line)
        d['name'], d['weight'], others = r.groups()
...
def calc_weight(df, name):
        idx = df.name == name
        tmp = df[idx]
        if tmp.shape[0] == 1:
                df.loc[idx, 'sum'] = tmp.weight.iloc[0]
                return tmp.weight.iloc[0]
        sum2 = 0
        for name in tmp.balance.values:
                sum2 += calc_weight(df, name)
        sum2 += tmp.weight.iloc[0]
        df.loc[idx, 'sum'] = sum2
        return sum2
...
def solve2(s, df):
    ...
    while True:
        tmp = df[df.name == root]
        root_children = tmp.balance.values
        tmp2 = df[df.name.isin(root_children)]
        tmp3 = tmp2.drop_duplicates(['name'])
        mismatch = tmp3['sum'].value_counts().reset_index()
        if len(mismatch) == 1:
                # print(root)
                # print(tmp)
                return tmp.weight.iloc[0] - mismatch_wt

        # print(root, mismatch['index'].values.tolist(), tmp3.weight.values.tolist(), mismatch_wt)
        mismatch_wt = mismatch['index'].diff().dropna().iloc[0]
        wt = mismatch['index'].iloc[-1]
        root = tmp3[tmp3['sum'] == wt].name.iloc[0]


print(solve2(*parse(inp)))